...
const getItemsFromNotas = notasM => notasM.map(notas => notas.$flapMap(nota => nota.itens));
const filterItemsByCode = (code, itemM) => itemsM.map(items => items.filter(item => item.codigo == code));
